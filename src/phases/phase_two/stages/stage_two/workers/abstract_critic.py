...
from ..prompts.abstract_critic_prompts import AbstractCriticPrompts
...
class AbstractCritic(CriticWorker):
    ...
    def __init__(self, config: Dict):
        ...
        self.prompts = AbstractCriticPrompts()
    ...
    def prepare_input(self, state: Dict[str, Any]) -> WorkerInput:
        ...
        return WorkerInput(
            outline_state=state,
            context={
                "abstract_framework": state.get("abstract_framework"),  # Changed from "current_abstract"
    ...
    def validate_output(self, output: WorkerOutput) -> bool:
        ...
        if not all(subsection in content for subsection in 
                  ["## Main Thesis", "## Core Contribution", "## Key Moves", "## Development Notes"]):
            return False
    ...
    def process_output(self, response: str) -> WorkerOutput:
        ...
        try:
            ...
            validation_section = response.split("# Validation Assessment")[1].split("#")[0]
            validation_results = {
    ...
    def _extract_framework_analysis(self, response: str) -> Dict[str, str]:
        ...
        try:
            framework_section = response.split("# Framework Components Analysis")[1].split("#")[0]
            components = {}
            
            for component in ["Main Thesis", "Core Contribution", "Key Moves", "Development Notes"]:
                if f"## {component}" in framework_section:
                    component_content = framework_section.split(f"## {component}")[1]
                    component_content = component_content.split("##")[0] if "##" in component_content else component_content
                    components[component.lower().replace(" ", "_")] = component_content.strip()
