...
class MoveDevelopmentWorker(DevelopmentWorker):
    ...
    def process_input(self, state: Dict[str, Any]) -> WorkerInput:
        ...
        if "framework" not in state or "outline" not in state or "key_moves" not in state:
        ...
        if current_move_development:
            print(f"Found current move development of type: {type(current_move_development)}")
        else:
            print(f"No current move development found - this is normal for initial phase.")
        
        # Get the specific move we're working on
        framework = state["framework"]
        moves_list = framework.get("key_moves", [])
        if move_index >= len(moves_list):
            raise ValueError(f"Invalid move index: {move_index}, only {len(moves_list)} moves available")
