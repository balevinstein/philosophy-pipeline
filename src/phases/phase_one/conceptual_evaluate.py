...
from src.phases.phase_one.prompts.conceptual_evaluate import TopicEvaluationPrompt
...
class ConceptualTopicEvaluator(BaseStage):
    ...
    def __init__(self):
        ...
        self.prompt_manager = TopicEvaluationPrompt(
            cull_min=self.cull_min, cull_max=self.cull_max
        )
    ...
    def run(self) -> Optional[Dict[str, Any]]:
        ...
        response = self.api_handler.make_api_call(
            stage="topic_evaluation", prompt=prompt
        )
        ...
        # Save and summarize
        self.json_handler.save_json(result, self.get_full_path("topics_culled"))
