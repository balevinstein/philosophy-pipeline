...
from src.phases.phase_one.prompts.conceptual_final_select import FinalSelectionPrompt
...
class FinalTopicSelector(BaseStage):
    ...
    def _make_selection_call(self, topics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt = self.prompt_manager.get_selection_prompt(json.dumps(topics, indent=2))

        response = self.api_handler.make_api_call(
            stage="topic_selection", prompt=prompt
        )

        result = json.loads(self.json_handler.clean_json_string(response))
    ...
    def _make_setup_call(self, selected_topic: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.api_handler.make_api_call(
            stage="phase_two_setup", prompt=prompt
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get full data for selected topic
            selected_topic = self._get_selected_topic_data(topics, selection_result)
            ...
            self.json_handler.save_json(
                final_result, self.config["paths"]["final_selection"]
            )
    ...
    def _print_setup_summary(self, result: Dict[str, Any]) -> None:
        ...
        areas = result["phase_two_setup"]["development_guidance"][
