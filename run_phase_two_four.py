...
from src.phases.phase_two.stages.stage_four.master_workflow import DetailedOutlineDevelopmentWorkflow


def load_config():
    """Load configuration from config/conceptual_config.yaml."""
    config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config", "conceptual_config.yaml")
...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "detailed_outline")
    ...
    framework_file = os.path.join(os.path.dirname(output_dir), "framework_development", "abstract_framework.json")
    outline_file = os.path.join(os.path.dirname(output_dir), "framework_development", "outline.json")
    key_moves_file = os.path.join(os.path.dirname(output_dir), "key_moves_development", "key_moves_development", "all_developed_moves.json")
    literature_file = os.path.join(os.path.dirname(output_dir), "literature_synthesis.json")
    
    # Add debug output
    print(f"\nChecking file paths:")
    print(f"framework_file: {framework_file} (exists: {os.path.exists(framework_file)})")
    print(f"outline_file: {outline_file} (exists: {os.path.exists(outline_file)})")
    print(f"key_moves_file: {key_moves_file} (exists: {os.path.exists(key_moves_file)})")
    print(f"literature_file: {literature_file} (exists: {os.path.exists(literature_file)})")
    ...
    if missing_files:
        print(f"ERROR: The following required input files are missing: {', '.join(missing_files)}")
    ...
    try:
        ...
        if os.path.exists(framework_file):
            ...
            with open(framework_file, "r") as f:
                ...
                if "abstract_framework" in framework_data:
                    ...
                    print(f"Successfully extracted inner abstract_framework with {len(framework)} keys")
                    print(f"Keys: {list(framework.keys())}")
                    print(f"Thesis preview: {framework.get('main_thesis', '')[:50]}...")
                else:
                    framework = framework_data
                    print("Warning: Could not find abstract_framework key in framework file")
        else:
        ...
        if os.path.exists(key_moves_file):
            ...
            raise FileNotFoundError(f"Developed key moves file not found: {key_moves_file}")
        ...
        if os.path.exists(literature_file):
            ...
            raise FileNotFoundError(f"Literature analysis file not found: {literature_file}")
